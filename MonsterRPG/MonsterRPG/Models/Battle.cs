...
namespace DragonWarrior.Models
{
    public class Battle
    {
        ...
        private void ExecuteAbility(BattleAction action)
        {
            ...
            action.Acting.Monster.UseAbility(action.Acting.ActionAbility, action.Receiving);

            if (action.Receiving.Status == Status.Fainted)
            {
                Console.WriteLine($"\n\n{action.Acting.Monster.Name} tried to {action.Acting.ActionAbility} {action.Receiving.Monster.Name}... but nothing happened!!");
            }
            else
            {
            ...
            foreach (var m in currentParty)
            {
                ...
                while(!turnOver)
                {
                    ...
                    switch (cki.Key)
                    {
                        ...
                        case ConsoleKey.D3:
                            if(m.Monster.Abilities.Count != 0)
                            {
                                MonsterUseAbility(m);
                                turnOver = true;
                            }
                            ...
                            {
                                Console.WriteLine($"\n\n{m.Monster.Name} has no abilities!!\n\n");
                                Console.ReadKey();
                            }
                            break;
        ...
        private void MonsterUseAbility(PartyMember m)
        {
            ...
            //the way to do this better is to get the input, pass to a method along with the list to be checked against, and return a bool if it's either
            //not an integer, or is outside the scope of the list. If it fails, you run the prompt again...

            ShowPartyAndFoe();

            Console.WriteLine($"\n\nWhich ability will {m.Monster.Name} use?\n\n");
            ...
            bool selectAbilityFail = true;

            while(selectAbilityFail)
            {
                if (!char.IsDigit(cki.KeyChar))
                {
                    ShowPartyAndFoe();
                    Console.WriteLine($"\n\nTry again. Which ability will {m.Monster.Name} use?\n\n");
                ...
                else
                {
                    ...
                    if (selection > m.Monster.Abilities.Count || selection < 1)
                    {
                        ShowPartyAndFoe();
                        Console.WriteLine($"\n\nTry again. Which ability will {m.Monster.Name} use?\n\n");
                    ...
                    {
                        selectAbilityFail = false;

                        string ability = m.Monster.Abilities[selection - 1];

                        ShowPartyAndFoe();
                        Console.WriteLine($"\n\nWho will {m.Monster.Name} {ability}?\n\n");
                        ...
                        foreach (var p in tempParty)
                        {
                            Console.WriteLine($"{selector}. {p.Monster.Name}");
                            selector++;
                        }

                        cki = Console.ReadKey();

                        bool selectRecipientFail = true;

                        while (selectRecipientFail)
                        {
                            if (!char.IsDigit(cki.KeyChar))
                            {
                                ShowPartyAndFoe();
                                Console.WriteLine($"\n\nTry Again. Who will {m.Monster.Name} {ability}?\n\n");
                            ...
                            {
                                selection = int.Parse(cki.KeyChar.ToString());

                                if (selection > tempParty.Count || selection < 1)
                                {
                                    ShowPartyAndFoe();
                                    Console.WriteLine($"\n\nTry Again. Who will {m.Monster.Name} {ability}?\n\n");
                                ...
                                {
                                    selectRecipientFail = false;

                                    PartyMember recipient = tempParty[selection - 1];

                                    BattleAction action = new BattleAction(m, recipient, ActionType.Ability);
                                    m.Action = action;
                                    m.ActionAbility = ability;
                                }
                            }
                        }
                    }                 
        ...
        private void MonsterAttack(PartyMember m)
        {
            ...
            while(fail)
            {
                ...
                else
                {
                    ...
                    if (selection > ComputerParty.Members.Count || selection < 1)
                    {
                        ShowPartyAndFoe();
                        Console.WriteLine($"\n\nTry again. Who will {m.Monster.Name} attack?\n\n");
                        ShowAttackOptions();
